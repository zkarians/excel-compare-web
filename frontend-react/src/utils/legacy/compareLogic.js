...
export const missingProductsSet = new Set();
export const weightMismatchSet = new Set();
export const weightMismatchDetails = {};
/**
 * 0 중량/치수 제품 및 지정 모델을 NONASSET.ITEM으로 취급할지 확인
 */
function checkIsNonAsset(prodName, productMap) {
    const cleanName = String(prodName || "").trim().toUpperCase();
...
function compareData(origList, downList, productMaster, dynamicRules, customFields, carrierMap, normalizeCarrier) {
    ...
    origList.forEach(orig => {
        let cleanCntr = String(orig.cntrNo || "").trim().toUpperCase();
        let effectiveProdName = String(orig.prodName || "").trim().toUpperCase();
    ...
    downList.forEach(down => {
        let cleanCntr = String(down.cntrNo || "").trim().toUpperCase();
        let cleanProdName = String(down.prodName || "").trim().toUpperCase();
    ...
    origList.forEach(o => {
        let cleanCntr = String(o.cntrNo || "").trim().toUpperCase();
    ...
    downKeys.forEach(key => {
        ...
        if (dynamicRules) {
            dynamicRules.forEach(rule => {
                ...
                const allMatched = rule.conditions[conditionMethod](cond => {
                    ...
                    else if (cond.field === 'prodName') targetText = String(evalProdName).toUpperCase().trim();
                    else if (cond.field === 'dest') targetText = String(evalDest).toUpperCase().trim();
                    else if (cond.field === 'downPort') targetText = String(downPort).toUpperCase().trim();
                    else if (cond.field === 'downForwarder') targetText = String(downForwarder).toUpperCase().trim();
                    else if (cond.field === 'downCarrier') targetText = String(downCarrier).toUpperCase().trim();
                    else if (cond.field === 'downRemark') targetText = String(downRemark).toUpperCase().trim();
                    else if (cond.field === 'origRemark') targetText = String(origRemark).toUpperCase().trim();
                    else if (cond.field === 'downLoadType') targetText = String(downLoadType).toUpperCase().trim();
                    else if (cond.field === 'downPlanQty') targetText = String(downPlanQty).toUpperCase().trim();
                    else if (cond.field === 'downPackingQty') targetText = String(downPackingQty).toUpperCase().trim();
                    else if (cond.field === 'downPendingQty') targetText = String(down.pendingQtySum || 0).toUpperCase().trim();
                    else if (cond.field === 'downSealNo') targetText = String(downSealNo).toUpperCase().trim();
                    else if (cond.field === 'prodType') {
                        targetText = matchedOrig ? String(matchedOrig.prodType || down.prodType || "").toUpperCase().trim() : String(down.prodType || "").toUpperCase().trim();
                    }
        ...
        if (!matchedOrig) {
            ...
            let origRemarkStr = String(matchedOrig.remark || "").trim();
            let downRemarkStr = String(down.remark || "").trim();
        ...
        const prod = productMap.get(String(down.prodName || "").trim().toUpperCase());
        ...
        let resultObj = {
            ...
            origRemark: (() => {
                ...
                if ((!r || String(r).trim().length <= 1) && downRemark && String(downRemark).trim().length > 1) {
        ...
        if (prod) {
            ...
            if (loadQty > 0 && Math.abs(downUnitWeight - dbWeight) > 0.05) {
                if (typeof weightMismatchSet !== 'undefined') weightMismatchSet.add(String(down.prodName || "").trim());
                resultObj.unitWeight = dbWeight;
            ...
            if (hasWeightDiff || hasCbmDiff) {
                const prodNameTrimmed = String(down.prodName || "").trim();
                if (typeof weightMismatchDetails !== 'undefined') {
            ...
            if (typeof missingProductsSet !== 'undefined') missingProductsSet.add(String(down.prodName || "").trim());
    ...
    origKeys.forEach(key => {
        if (!downGrouped[key]) {
            ...
            const cleanCntr = String(orig.cntrNo || "").trim().toUpperCase();
            const prodNameUpper = String(orig.prodName || "").trim().toUpperCase();
            ...
            if (!isNonAsset) {
                stats.extra++;
                if (!prod && typeof missingProductsSet !== 'undefined') missingProductsSet.add(String(orig.prodName || "").trim());
            }
...
export { compareData, evaluateMathString, checkIsNonAsset };
