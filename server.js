...
// CTNR Worker DB Pool (CockroachDB 근무편성 및 조 운영계획 조회용)
let workerPool = null;
...
function getWorkerPool() {
    if (!workerPool) {
        workerPool = new Pool({
            connectionString: process.env.WORKER_DATABASE_URL || DEFAULT_WORKER_DB_URL,
...
const ROSTER_WEEKDAYS = ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'];

function formatRosterDateStr(d) {
...
async function getUpcoming3DaysRosterStatus(targetDateStr) {
    try {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            const dateStr = formatRosterDateStr(d);
            targetDates.push({
                date: d,
                dateStr,
                day: d.getDate(),
                weekday: ROSTER_WEEKDAYS[d.getDay()]
            });
        }
...
app.get('/api/reports/roster-status', async (req, res) => {
    try {
        const targetDate = req.query.date || getReportWorkDateString(new Date());
        const rosterResult = await getUpcoming3DaysRosterStatus(targetDate);
        return res.json(rosterResult);
    } catch (err) {
        console.error("GET /api/reports/roster-status error:", err);
...
app.get('/api/reports/generate', async (req, res) => {
    try {
        ...
        try {
            ...
            const rosterResult = await getUpcoming3DaysRosterStatus(targetDateStr);
            const reportText = buildReportTextFromData(reportData, 'full');
            return res.json({
                success: true,
                reportData,
                reportText,
                upcomingRosterText: rosterResult.formattedText || '',
                upcomingRosterMessages: rosterResult.messages || []
            });
